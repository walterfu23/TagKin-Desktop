...
Future<void> _createPersonInPicker(WidgetTester tester, String name) async {
  expect(find.byKey(const Key('person-picker-dialog')), findsOneWidget);
  await tester.enterText(find.byKey(const Key('person-picker-search')), name);
...
void main() {
  ...
  testWidgets('face crop trays: loads unassigned + excluded columns', (
    ...
    expect(
      find.text('faces should not be assigned to a person'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'face crop trays: returning to Faces re-syncs after a server assign',
    (tester) async {
      ...
      // Same as item-detail Save: API mutated while Faces stayed mounted.
      await persons.reassignAppearance('ap_u', personId: 'person_1');
      container.read(activeTopLevelTabProvider.notifier).state =
          TopLevelTab.persons;
      await tester.pump();
      container.read(activeTopLevelTabProvider.notifier).state =
  ...
  testWidgets('face crop trays: unassigned + excluded tiles show Image crops', (
    tester,
  ) async {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        tester
            .widget<Text>(find.byKey(const Key('face-crop-person-name')))
            .data,
        'Sam Updated',
      );
      expect(
  ...
  testWidgets('face crop trays: rename in folder2 splits shared person', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
    ...
    final minted = persons.personDetails.singleWhere(
      (p) => p.name == 'Person2',
    );
    expect(minted.appearances.map((a) => a.id), ['faces2']);
  });

  testWidgets('face crop trays: rename collision merges folder faces only', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
    ...
    expect(
      target.appearances.map((a) => a.id),
      containsAll(['faces_p2', 'faces2']),
    );
    expect(
      persons.personDetails.where((p) => p.id.startsWith('person_new')),
      isEmpty,
    );
  });

  testWidgets('face crop trays: rename collision other name stays in rename', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
  ...
  testWidgets('face crop trays: Set name from unassigned loose crop', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          personsRepositoryProvider.overrideWithValue(persons),
          itemsRepositoryProvider.overrideWithValue(_itemsInAlbum(['item_u'])),
        ],
    ...
    expect(find.byKey(const Key('person-picker-dialog')), findsOneWidget);
    await _createPersonInPicker(tester, 'Riley');
  ...
  testWidgets(
    'face crop trays: Set name assigns a Folder2 face to an existing '
    'out-of-folder person without typing',
    (tester) async {
      final persons = FakePersonsRepository(
        persons: [
          fixturePersonDetail(
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(
            home: FaceCropTraysPage(initialLeafFolder: '/albums/Folder2'),
          ),
        ),
      );
      await tester.pumpAndSettle();

      await tester.tap(find.byKey(const Key('face-crop-set-name-ap_f2')));
      await tester.pumpAndSettle();

      expect(find.byKey(const Key('person-picker-dialog')), findsOneWidget);
      expect(
        find.byKey(const Key('person-picker-option-person_1')),
        findsOneWidget,
      );
      ...
      expect(persons.reassignCalls, hasLength(1));
      expect(persons.reassignCalls.single.appearanceId, 'ap_f2');
  ...
  testWidgets(
    'face crop trays: Set name greys a person already on this photo',
    (tester) async {
      final persons = FakePersonsRepository(
        persons: [
          fixturePersonDetail(
      ...
      persons.unassignedAppearances.add(
        fixtureAppearance(
          ...
          itemId: 'item_same',
          tagId: 'tag_b',
          region: const TagRegion(yMin: 0.2, xMin: 0.5, yMax: 0.5, xMax: 0.8),
        ),
      );
      ...
      await tester.tap(find.byKey(const Key('face-crop-set-name-ap_b')));
      await tester.pumpAndSettle();
  ...
  testWidgets('face crop trays: click selects; Cmd+click multi-selects', (
    tester,
  ) async {
    final persons = FakePersonsRepository(persons: const []);
    ...
    final u2 = tester.getRect(
      find.byKey(const Key('face-crop-appearance-ap_u2')),
    );
  ...
  testWidgets('face crop trays: Shift+click selects an inclusive range', (
    tester,
  ) async {
    final persons = FakePersonsRepository(persons: const []);
    persons.unassignedAppearances.addAll([
    ...
    final r1 = tester.getRect(
      find.byKey(const Key('face-crop-appearance-ap_r1')),
    );
    ...
    final r3 = tester.getRect(
      find.byKey(const Key('face-crop-appearance-ap_r3')),
    );
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(
            home: SelectionArea(child: FaceCropTraysPage()),
          ),
        ),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final g1 = tester.getRect(
        find.byKey(const Key('face-crop-appearance-ap_g1')),
      );
      await tester.tapAt(Offset(g1.left + 10, g1.bottom - 10));
      await tester.pumpAndSettle();
      debugFaceCropMetaPressed = () => true;
      final g2 = tester.getRect(
        find.byKey(const Key('face-crop-appearance-ap_g2')),
      );
      ...
      expect(
        find.byKey(const Key('face-crop-group-selection')),
        findsOneWidget,
      );
      ...
      expect(persons.assembleAppearancesCalls.single.toSet(), {
        'ap_g1',
        'ap_g2',
      });
      expect(
      ...
      expect(
        find.byKey(const Key('face-crop-ungroup-fg_fm_1')),
        findsOneWidget,
      );
  ...
  testWidgets('face crop trays: Excluded Cmd+click multi-selects loose faces', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1400, 900);
    tester.view.devicePixelRatio = 1.0;
    ...
    final e1 = tester.getRect(
      find.byKey(const Key('face-crop-exclusion-ex_m1')),
    );
    ...
    debugFaceCropMetaPressed = () => true;
    final e2 = tester.getRect(
      find.byKey(const Key('face-crop-exclusion-ex_m2')),
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final g1 = tester.getRect(
        find.byKey(const Key('face-crop-exclusion-ex_g1')),
      );
      await tester.tapAt(Offset(g1.left + 10, g1.bottom - 10));
      await tester.pumpAndSettle();
      debugFaceCropMetaPressed = () => true;
      final g2 = tester.getRect(
        find.byKey(const Key('face-crop-exclusion-ex_g2')),
      );
      await tester.tapAt(Offset(g2.left + 10, g2.bottom - 10));
      await tester.pumpAndSettle();
      debugFaceCropMetaPressed = null;
      ...
      expect(persons.assembleExclusionsCalls.single.toSet(), {
        'ex_g1',
        'ex_g2',
      });
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final moved = persons.unassignedAppearances
          .where((a) => a.id == 'ap_a')
          .single;
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.byKey(const Key('person-picker-dialog')), findsOneWidget);
      await _createPersonInPicker(tester, 'Sam');
  ...
  testWidgets(
    'face crop trays: New person in the Set name picker names a Similar-faces group',
    (tester) async {
      final persons = FakePersonsRepository(
        persons: [
          fixturePersonDetail(
      ...
      // New person is offered even though an existing person is listed and
      // nothing has been typed into the search box.
      expect(
        find.byKey(const Key('person-picker-option-person_1')),
        findsOneWidget,
      );
      ...
      expect(persons.assignFaceGroupCalls, hasLength(1));
      expect(persons.assignFaceGroupCalls.single.faceGroupId, 'fg_1');
      expect(persons.assignFaceGroupCalls.single.name, 'Riley');
      expect(persons.assignFaceGroupCalls.single.personId, isNull);
      expect(find.byKey(const Key('face-crop-facegroup-fg_1')), findsNothing);
      expect(
        tester
            .widget<Text>(find.byKey(const Key('face-crop-person-name')))
  ...
  testWidgets(
    'face crop trays: Set name on a same-photo Similar-faces group names the closest and skips the rest',
    (tester) async {
      tester.view.physicalSize = const Size(1400, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      persons.unassignedAppearances.addAll([
        ...
        fixtureAppearance(
          ...
          itemId: 'item_same',
          tagId: 'tag_skip',
          region: const TagRegion(yMin: 0.1, xMin: 0.5, yMax: 0.4, xMax: 0.8),
        ),
        fixtureAppearance(
          id: 'ap_other',
          personId: null,
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            personsRepositoryProvider.overrideWithValue(persons),
            itemsRepositoryProvider.overrideWithValue(
              _itemsInAlbum(['item_same', 'item_other']),
            ),
          ],
          child: const MaterialApp(home: FaceCropTraysPage()),
        ),
      ...
      await tester.tap(
        find.byKey(const Key('face-crop-facegroup-set-name-fg_same')),
      );
      await tester.pumpAndSettle();
      await _createPersonInPicker(tester, 'Sam');
      ...
      expect(persons.assignFaceGroupCalls, hasLength(1));
      expect(
        find.byKey(const Key('face-crop-facegroup-fg_same')),
        findsNothing,
      );
      expect(find.byKey(const Key('face-crop-person-name')), findsOneWidget);
      expect(
      ...
      expect(persons.unassignedAppearances.map((a) => a.id), ['ap_skip']);
      expect(
        find.byKey(const Key('face-crop-group-skipped-same-photo')),
        findsOneWidget,
      );
      expect(
        find.text(
  ...
  testWidgets('face crop trays: Remove person moves faces to Unassigned', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          personsRepositoryProvider.overrideWithValue(persons),
          itemsRepositoryProvider.overrideWithValue(_itemsInAlbum(['item_a'])),
        ],
        child: const MaterialApp(
          home: FaceCropTraysPage(initialPersonId: 'person_1'),
        ),
    ...
    await tester.pumpAndSettle();
    await tester.tap(
      find.byKey(const Key('face-crop-person-unassign-confirm')),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('face crop trays: folder picker scopes unassigned crops', (
    tester,
  ) async {
    final persons = FakePersonsRepository(persons: const []);
    persons.unassignedAppearances.addAll([
    ...
    expect(
      find.byKey(const Key('face-crop-appearance-ap_paris')),
      findsOneWidget,
    );
    ...
    expect(
      find.byKey(const Key('face-crop-appearance-ap_rome')),
      findsOneWidget,
    );
    expect(
      find.byKey(const Key('face-crop-appearance-ap_paris')),
  ...
  testWidgets('face crop trays: first load selects first in-folder person', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('face-crop-person-option-new')),
        findsOneWidget,
      );
      expect(
      ...
      expect(
        find.byKey(
          const Key('face-crop-person-option-in-folder-person_rome_only'),
        ),
        findsNothing,
      );
  ...
  testWidgets('face crop trays: folder switch selects first in-folder person', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
  ...
  testWidgets('face crop trays: assigned likeness groups are boxed by person', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
    ...
    await tester.tap(
      find.byKey(const Key('face-crop-cluster-header-person_a')),
    );
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('face-crop-selected-ap_a1')), findsOneWidget);
    expect(find.byKey(const Key('face-crop-selected-ap_a2')), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('face-crop-cluster-person_a')),
        findsOneWidget,
      );
      expect(find.byKey(const Key('face-crop-cluster-person_b')), findsNothing);
      expect(find.byKey(const Key('face-crop-appearance-ap_b1')), findsNothing);
      ...
      expect(
        find.byKey(const Key('face-crop-cluster-person_b')),
        findsOneWidget,
      );
  ...
  testWidgets('face crop trays: drag cluster header creates one GroupFM', (
    tester,
  ) async {
    final persons = FakePersonsRepository(
      persons: [
    ...
    expect(persons.unlinkCalls, isEmpty);
    expect(persons.unassignAppearancesCalls, hasLength(1));
    expect(persons.unassignAppearancesCalls.single.toSet(), {'ap_g1', 'ap_g2'});
    expect(persons.unassignedAppearances.map((a) => a.id).toSet(), {
      'ap_g1',
      'ap_g2',
    });
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('face-crop-appearance-ap_b')),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(persons.accountExclusions.map((e) => e.faceGroupId).toSet(), {
        'fg_1',
      });
      expect(
        find.descendant(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        persons.accountExclusions.every(
          (e) => e.faceGroupKind == FaceGroupKind.fm,
        ),
        isTrue,
      );
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(
          Key('face-crop-selected-${_exclusionIdForTag(persons, 'tag_u1')}'),
        ),
        findsOneWidget,
      );
      expect(
        find.byKey(
          Key('face-crop-selected-${_exclusionIdForTag(persons, 'tag_u2')}'),
        ),
        findsOneWidget,
      );
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.byKey(Key('face-crop-selected-$exclU1')), findsOneWidget);
      expect(find.byKey(Key('face-crop-selected-$exclU2')), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(persons.unlinkCalls, isEmpty);
      expect(persons.unassignAppearancesCalls, hasLength(1));
      expect(persons.unassignAppearancesCalls.single.toSet(), {
        'ap_a1',
        'ap_a2',
      });
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.textContaining('Select a person before assigning'),
        findsNothing,
      );
      expect(persons.unassignedAppearances, isEmpty);
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(persons.confirmAppearanceCalls.toSet(), {'ap_new1', 'ap_new2'});
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(persons.declineAutoAssignCalls.toSet(), {'ap_new1', 'ap_new2'});
      expect(persons.declineAutoAssignAppearancesCalls, hasLength(1));
      expect(persons.declineAutoAssignAppearancesCalls.single.toSet(), {
        'ap_new1',
        'ap_new2',
      });
  ...
  testWidgets('face crop trays: person dropdown flags unconfirmed faces '
      'in the current folder only', (tester) async {
    final persons = FakePersonsRepository(
      persons: [
        fixturePersonDetail(
          id: 'person_1',
  ...
  testWidgets(
    'face crop trays: person dropdown is A–Z including out-of-folder selection',
    (tester) async {
      final persons = FakePersonsRepository(
        persons: [
          fixturePersonDetail(
      ...
      final items = FakeItemsRepository(
        items: [
          fixtureItem(
            id: 'item_trip_z',
            sourceRef: 'file:///albums/Trip/z.jpg',
            processingStatus: ProcessingStatus.tagged,
            contentHash: null,
          ),
          fixtureItem(
            id: 'item_trip_m',
            sourceRef: 'file:///albums/Trip/m.jpg',
            processingStatus: ProcessingStatus.tagged,
            contentHash: null,
          ),
          fixtureItem(
            id: 'item_trip_u',
            sourceRef: 'file:///albums/Trip/u.jpg',
            processingStatus: ProcessingStatus.tagged,
            contentHash: null,
          ),
          fixtureItem(
            id: 'item_rome',
            sourceRef: 'file:///albums/Rome/a.jpg',
            processingStatus: ProcessingStatus.tagged,
            contentHash: null,
          ),
        ],
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(
            home: FaceCropTraysPage(
              initialPersonId: 'person_a',
              initialLeafFolder: '/albums/Trip',
            ),
          ),
        ),
      );
      ...
      expect(button.items!.map((item) => item.value).toList(), [
        '__new_person__',
        'person_a',
        'person_m',
        'person_z',
  ...
  testWidgets(
    'face crop trays: Group of two faces from the same photo shows why',
    (tester) async {
      tester.view.physicalSize = const Size(1400, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final g1 = tester.getRect(
        find.byKey(const Key('face-crop-appearance-ap_same_1')),
      );
      await tester.tapAt(Offset(g1.left + 10, g1.bottom - 10));
      await tester.pumpAndSettle();
      debugFaceCropMetaPressed = () => true;
      final g2 = tester.getRect(
        find.byKey(const Key('face-crop-appearance-ap_same_2')),
      );
      await tester.tapAt(Offset(g2.left + 10, g2.bottom - 10));
      await tester.pumpAndSettle();
      debugFaceCropMetaPressed = null;
      ...
      expect(persons.assembleAppearancesCalls, isEmpty);
      expect(
        find.byKey(const Key('face-crop-group-same-photo')),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'face crop trays: drop onto a person skips a face already sharing that photo',
    (tester) async {
      tester.view.physicalSize = const Size(1400, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            personsRepositoryProvider.overrideWithValue(persons),
            itemsRepositoryProvider.overrideWithValue(
              _itemsInAlbum(['item_1']),
            ),
          ],
          child: const MaterialApp(
            home: FaceCropTraysPage(initialPersonId: 'person_maya'),
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      final from = tester.getCenter(
        find.byKey(const Key('face-crop-appearance-ap_other')),
      );
      final to = tester.getCenter(
        find.byKey(const Key('face-crop-tray-assigned')),
      );
      ...
      expect(persons.reassignCalls, isEmpty);
      expect(
        find.byKey(const Key('face-crop-already-on-photo')),
        findsOneWidget,
      );
    },
  );
