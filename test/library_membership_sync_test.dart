...
void main() {
  test(
    'publish adopts pending leaf even when table statusFilter is tagged',
    () async {
      ...
      final store = MemoryCollectionsStore(
        const CollectionsFile(
          collections: [
            Collection(id: 'c1', name: 'Grow', leafFolders: ['/albums/Alpha']),
          ],
      ...
      await table.load();
      expect(table.allRows.map((r) => r.item.id), ['a1']);
  ...
  test(
    'publish does not shrink membership when leaf missing from filter',
    () async {
      final items = FakeItemsRepository(
        items: [
          fixtureItem(
  ...
  test(
    'publish claims leaf owned by another collection under ingest root',
    () async {
      final items = FakeItemsRepository(
        items: [
          fixtureItem(
      ...
      await publishCollectionMembershipFromLibrary(
        ...
        stealFolders: {'/Users/w/test/20260508'},
      );
      ...
      expect(table.collectionLeafFolders, contains('/Users/w/test/20260508'));
    },
  );

  test('publish does not steal owned leaf without stealFolders', () async {
    final items = FakeItemsRepository(
      items: [
        fixtureItem(
          id: 'a1',
        ...
        fixtureItem(
          id: 'c1',
          sourceRef: 'file:///albums/New/1.jpg',
          processingStatus: ProcessingStatus.pending,
          contentHash: 'h3',
        ),
      ],
    ...
    await publishCollectionMembershipFromLibrary(
      ...
      cols: cols,
      table: table,
      claimUnderFolder: '/albums',
    );

    expect(
      cols.current.leafFolders.toSet(),
      containsAll(['/albums/Alpha', '/albums/New']),
    );
  ...
  test(
    'publish claims ingest root into origin collection not the open one',
    () async {
      final items = FakeItemsRepository(
        items: [
          fixtureItem(
      ...
      final store = MemoryCollectionsStore(
        const CollectionsFile(
          collections: [
            Collection(id: 'c1', name: 'First', leafFolders: ['/albums/Old']),
            Collection(id: 'c2', name: 'Second', leafFolders: []),
          ],
